...
1. Edit the configuration file (`Config_clusters.ini` or `Config_pairwise.ini`)
   to suit your data. A guide is embedded within each configuration file.
...
script will group the data by cluster and data source, and keep only the unique
information from each data source per cluster.
